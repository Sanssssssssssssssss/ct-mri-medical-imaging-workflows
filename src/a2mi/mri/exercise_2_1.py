...
def _robust_normalize(image: np.ndarray, lower_pct: float = 1.0, upper_pct: float = 99.5) -> np.ndarray:
    """Normalize one image with percentile clipping for display only.

    Args:
        image: Input image to normalize.
        lower_pct: Lower clipping percentile.
        upper_pct: Upper clipping percentile.

    Returns:
        A float image scaled to ``[0, 1]`` for visualization.
    """
    x = np.asarray(image, dtype=np.float32)
    lo = float(np.percentile(x, lower_pct))
    hi = float(np.percentile(x, upper_pct))
    if hi <= lo:
        lo = float(np.min(x))
        hi = float(np.max(x))
    if hi <= lo:
...
def plot_image_magnitude_all_coils_adaptive(
    img_coils: np.ndarray,
    out_path: str | Path | None = None,
    show: bool = True,
    lower_pct: float = 1.0,
    ...
    """Plot coil magnitudes with per-coil adaptive contrast for noise inspection.

    This figure is intended for qualitative comparison only. Each coil is
    independently normalized so that background noise, striping, and other
    coil-specific artefacts are easier to inspect.

    Args:
        img_coils: Coil-first reconstructed image data.
        out_path: Optional file path for saving the figure.
        show: Whether to display the figure interactively.
        lower_pct: Lower percentile used for clipping.
        upper_pct: Upper percentile used for clipping.
    """
    n_coils = int(img_coils.shape[0])
    rows, cols = _grid(n_coils, default_cols=3)
    fig, axes = plt.subplots(rows, cols, figsize=(3.2 * cols, 3.0 * rows), constrained_layout=True)
    ...
    for c in range(n_coils):
        magnitude = np.abs(img_coils[c])
        display_img = _robust_normalize(magnitude, lower_pct=lower_pct, upper_pct=upper_pct)
        axes[c].imshow(display_img, cmap="gray", vmin=0.0, vmax=1.0)
        axes[c].set_title(f"Coil {c} | adaptive")
        axes[c].axis("off")
